...
"KAI NOTES"
"SciKitLearn no longer useful anymore for NN, TensorFlow, Keras & PyTorch is our new jam"
"Google released in 2015, framework for large scale training"
"Keras is now a part of TensorFlow, high level, user friendly, quick prototyping and model building"
"PyTorch came after TensorFlow, designed to be more pythonic, flexible and easier to use"

# For reproducability
np.random.seed(42)
keras.utils.set_random_seed(42) #Set the random state for good 

# Load data
"fashion is a famous data set that comes with keras "
...
"We no longer need to stratify because the dataset is so large"
...
"We read an image which greyscales it, and we get a value for each pixel, either 0-255, which represents"
"the colour of it"
...
"Flatten stretches the whole image into a single vector"
"Set to activation = relu and # of neurons?"
"Dropout is a regularization technique, its a layer that 20% says that for each of the connections, randomly set them to 0. This avoids overfitting and reduces interdependecies "
"Output layer, we know there are 10 diff classes. and we pick Softmax (for classification)?"
