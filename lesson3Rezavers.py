...
#lack of reset index broke the code on Mon Sep 15 
...
#corr value = 1 is perfect linear correlation, which will confuse a learning model.
#the diagonal is perectly linear because every data set is proportional to itself
#Here, we are masking any corr value that is less than 0.8, such that we can see what is useful to us
...
#here, he is comparing y(house value) to x categories
...
#X_train.to.csv("UnscaledOriginalData.csv")
X_train = sc.transform(X_train)
#X_train.to.csv("NowScaledData.csv")
#this scaled/normalized data does not introduce bias into the model because everything is
#in a common range of [-1, 1]. I guess its normalized based off the min/max of a data set?
...
mdl1 = LinearRegression() #create instance of LR model
mdl1.fit(X_train, y_train)
#linear regression model is most basic ML model. 
...
print("Model 1 training MAE is: ", round(mae_train1,2)) #MAE = Mean Abs. Error
#us measuring the performance of our system, using absolute error as a ref.
...
y_pred_test2 = mdl1.predict(X_test)
mae_test2 = mean_absolute_error(y_test, y_pred_test2)
print("Model 2 Test MAE:", round(mae_test2, 2))


# We better use k-fold cross validation for more robust evaluation. for model 1 
...
# We better use k-fold cross validation for more robust evaluation. for model 2
...
#there is a data leak above in the two implementarion because all of the training data is normalized 
# when 'the test data set should be excluded'?

# Pipeline - This is how we prevent a data leak
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
pipeline1 = Pipeline([
    ('scaler', StandardScaler()),
    ('model', LinearRegression())]) #it scales first and then applies linear scaling
...
pipeline2 = Pipeline([
    ('scaler', StandardScaler()), #change estimator to random numbers for manual tuning
...
# Using grid search - is a manual way to tune your estimator to your model
...
grid = GridSearchCV(
    ...
    cv=cv,
    n_jobs=-1, #you can parallelize multiple threads of your CPU to make this faster, n_jobs ++
    refit=True,           
    verbose=1,
...
#QUiz 
#what is the leak here?
#Why do we do K fold cross validation?
#HOw many operations are we doing here? 
